...
class SpreadSheet_Connect:
    ...
    def data_input_chehum(self, input_list):
        #all_data = np.array(self.chehumsheet.get_all_values())
        ...
        if input_list[0][4] == "":
            ...
            self.chehumsheet.update_cell(last_row, 10, value="=\"Daugu-2024-\"&text(H" + str(last_row) + ",\"00#\")")
            st.success(message.format(input_list[0][0], input_list[0][2], input_list[0][3], input_list[0][4],
                                      input_list[0][5]))

    def serial_input_chehum(self):

        serials = self.chehumsheet.get_values("I:I")
        print(type(serials))
        print(serials)
        approvals = self.chehumsheet.get_values("I:I")
        print(type(approvals))
        print(approvals)
